 ...
 * It handles user registration, email verification, login, token refresh, and logout.
 * It uses various services to manage users, JWT tokens, TOTP, and verification tokens.
 */
@Slf4j
@Service
...
public class AuthServiceImpl implements AuthService {
  ...
  public AuthResponse register(RegisterRequest registerRequest) {
    ...
    return AuthResponse.success(
            "Please scan the QR code with Google Authenticator"
                    + "and enter the code to complete registration.",
            responseData
    );
  }
  ...
  public AuthResponse verifyEmail(EmailVerificationRequest verificationRequest) {
    ...
    String secretKey = verificationTokenService.getSecretKeyByUser(user)
            .orElseThrow(() -> new AuthException("No TOTP setup found for user."
  ...
  public JwtResponse refreshToken(RefreshTokenRequest refreshTokenRequest) {
    ...
    return new JwtResponse(
            newAccessToken,
            newRefreshToken,
            EXPIRES_IN, // 15 minutes in seconds
            user.getId().toString(),
            user.getEmail(),
            user.getRoles()
    );
  ...
  /**
   * Utility method to validate access token.
   */
  ...
  /**
   * Utility method to extract user from access token.
   */
  @Transactional(readOnly = true)
