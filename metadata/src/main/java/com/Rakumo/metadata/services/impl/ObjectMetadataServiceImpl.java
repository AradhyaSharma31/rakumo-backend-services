...
public class ObjectMetadataServiceImpl implements ObjectMetadataService {
  ...
  @Override
  @Transactional
  public ObjectMetadataDto createObjectMetadata(UUID objectId, UUID bucketId, ObjectMetadata objectMetadata) {
    ...
    newObjectMetadata.setId(objectId);
    newObjectMetadata.setBucket(bucket);
    newObjectMetadata.setObjectKey(objectMetadata.getObjectKey());
    newObjectMetadata.setLatestVersionId(objectMetadata.getLatestVersionId());
    newObjectMetadata.setLatestSize(objectMetadata.getLatestSize());
    newObjectMetadata.setLatestEtag(objectMetadata.getLatestEtag());
    newObjectMetadata.setCreatedAt(Instant.now());
    newObjectMetadata.setUpdatedAt(Instant.now());
    newObjectMetadata.setIsDeleted(false);

    ObjectMetadata savedObject = objectMetadataRepo.save(newObjectMetadata);
  ...
  @Override
  @Transactional
  public ObjectMetadataDto updateObject(UUID objectId, UUID bucketId, ObjectMetadata objectMetadata)
    ...
    if (objectMetadata.getLatestVersionId() != null) {
      object.setLatestVersionId(object.getLatestVersionId());
    }
    if (objectMetadata.getLatestEtag() != null) {
      object.setLatestEtag(object.getLatestEtag());
    }
    if (objectMetadata.getLatestSize() != null) {
      object.setLatestSize(object.getLatestSize());
    }
    if (objectMetadata.getIsDeleted() != null) {
      object.setIsDeleted(objectMetadata.getIsDeleted());
    }
    object.setUpdatedAt(Instant.now());

    ObjectMetadata updatedObject = objectMetadataRepo.save(object);
    LOGGER.warning("Object " + objectMetadata.getId() + " updated successfully");
